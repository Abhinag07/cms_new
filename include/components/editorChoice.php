...
INNER JOIN `editor_choice` ON `stories`.`post_uid`=`editor_choice`.`post_uid`  WHERE `stories`.`post_status` = 'published' AND `stories`.`unlisted` = 'false'  ORDER BY `editor_choice`.`ec_status` ='active' LIMIT 10";
